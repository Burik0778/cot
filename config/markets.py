...
class Market:
    ...
    fred_series: Optional[str] = None       # серии FRED через '|', пробуются по очереди
    price_is_inverse: bool = False  # True: рост пары = ослабление инструмента (USD в базе)
    stooq_symbol: Optional[str] = None      # запасной источник: символы Stooq через '|'
    pip: float = 0.0001                     # размер одного пункта в единицах цены
    unit: str = "пп"                        # как называть эту единицу в интерфейсе
...
MARKETS: list[Market] = [
    ...
    Market("JPY", "Японская иена", "FX", TFF, "JAPANESE YEN", "USDJPY", "DEXJPUS", True, pip=0.01),
    ...
    Market("NZD", "Новозеландский доллар", "FX", TFF, "NEW ZEALAND DOLLAR", "NZDUSD", None, stooq_symbol="nzdusd"),
    Market("MXN", "Мексиканское песо", "FX", TFF, "MEXICAN PESO", "USDMXN", "DEXMXUS", True),
    Market("DXY", "Индекс доллара", "FX", TFF, "USD INDEX|U.S. DOLLAR INDEX|DOLLAR INDEX", "DXY", None, stooq_symbol="dx.f|usdidx", pip=0.01, unit="пунктов"),

    # ─── Индексы (TFF) ───────────────────────────────────────────────────
    Market("SP500", "S&P 500", "INDICES", TFF, "E-MINI S&P 500|E-MINI S&P500|S&P 500 Consolidated|S&P 500", "SPX", "SP500", pip=1.0, unit="пунктов"),
    Market("NASDAQ", "Nasdaq 100", "INDICES", TFF, "NASDAQ-100 Consolidated|NASDAQ-100 STOCK INDEX|E-MINI NASDAQ|NASDAQ", "NDX", "NASDAQ100", pip=1.0, unit="пунктов"),
    Market("DOW", "Dow Jones", "INDICES", TFF, "DJIA Consolidated|DOW JONES|DJIA", "DJI", "DJIA", pip=1.0, unit="пунктов"),
    Market("RUSSELL", "Russell 2000", "INDICES", TFF, "RUSSELL E-MINI|E-MINI RUSSELL 2000|RUSSELL 2000", "RUT", None, stooq_symbol="^rut", pip=1.0, unit="пунктов"),
    Market("VIX", "Индекс волатильности VIX", "INDICES", TFF, "VIX FUTURES|CBOE VOLATILITY|VOLATILITY INDEX", "VIX", "VIXCLS", pip=0.01, unit="пунктов"),

    # ─── Крипта (TFF) ────────────────────────────────────────────────────
    Market("BTC", "Биткоин", "CRYPTO", TFF, "BITCOIN", "BTCUSD", "CBBTCUSD", pip=1.0, unit="$"),
    Market("ETH", "Эфир", "CRYPTO", TFF, "ETHER CASH SETTLED|ETHER", "ETHUSD", "CBETHUSD", pip=1.0, unit="$"),

    # ─── Металлы (DISAGGREGATED) ─────────────────────────────────────────
    Market("GOLD", "Золото", "METALS", DISAGGREGATED, "GOLD - COMMODITY EXCHANGE|GOLD", "XAUUSD", None, stooq_symbol="xauusd", pip=1.0, unit="$"),
    Market("SILVER", "Серебро", "METALS", DISAGGREGATED, "SILVER - COMMODITY EXCHANGE|SILVER", "XAGUSD", None, stooq_symbol="xagusd", pip=0.01, unit="центов"),
    Market("COPPER", "Медь", "METALS", DISAGGREGATED, "COPPER- #1|COPPER", "COPPER", None, stooq_symbol="hg.f", pip=0.01, unit="центов"),
    Market("PLATINUM", "Платина", "METALS", DISAGGREGATED, "PLATINUM", "XPTUSD", None, stooq_symbol="xptusd", pip=1.0, unit="$"),

    # ─── Ставки (TFF) ────────────────────────────────────────────────────
    Market("UST10Y", "Гособлигации США 10 лет", "RATES", TFF, "UST 10Y NOTE|10-YEAR U.S. TREASURY|10 YEAR NOTE", "US10Y", "DGS10", pip=0.01, unit="б.п."),
    Market("UST2Y", "Гособлигации США 2 года", "RATES", TFF, "UST 2Y NOTE|2-YEAR U.S. TREASURY|2 YEAR NOTE", "US2Y", "DGS2", pip=0.01, unit="б.п."),
]
